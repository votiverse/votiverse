...
- **An open vote cannot be aborted.** Once voting has started, a voting event ends only by reaching `votingEnd`; whether it yields a result is decided by `BallotConfig.quorum`. There is no admin "stop"/"abort" for an in-progress vote — this keeps cast votes and declared results tamper-evident. Votes are cancellable only *before* voting opens (deliberation/curation) via `engine.events.cancelEvent`, which is terminal and admin-gated. The recourse for a compromised or contested live vote is the immutable record plus a follow-up vote, not an admin override. Ratified 2026-07-10; rationale in `docs/design/vote-lifecycle-abort-and-archive.md`.
