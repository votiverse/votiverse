...
| `users` | No | Undocumented addition — stores PII |
| `participants` | No (participants were expected to be opaque IDs) | Undocumented — stores names |
...
|--------|-------------|-------|
| `X-Participant-Id` | Partially (mentioned in CLAUDE.md) | Direct participant identity |
| `X-User-Id` | No | Cross-assembly user resolution |
...
**R1. Remove PII from the VCP.** Implement the identity boundary changes described in Section 4. Remove the `users` table, remove `name` from `participants`, move the identity resolver to the client. This is the single most important change — it restores the foundational privacy guarantee.

**R2. Enforce sovereignty on all write operations.** Add `requireParticipant` middleware to `POST /votes` and `POST /polls/:pid/respond`. These endpoints should take the participant ID from the authenticated context (`c.get("participantId")`), not from the request body. The request body should contain only the governance data (issueId, choice, answers).

### Priority 2: High (should fix before multi-tenant deployment)

**R3. Remove or assembly-scope the users endpoints.** If the users table is retained for any reason (see R1), the `/users` endpoints must be gated behind an `operational` scope and must not be accessible to regular clients. A regular client should never be able to list all users across all assemblies.

**R4. Make CORS configurable.** The `origin: (origin) => origin` pattern should be replaced with a configurable allowlist. In development, `*` is fine. In production, only the specific client origins should be allowed.

**R5. Standardize delegation visibility defaults.** Extract the fallback `{ mode: "public", incomingVisibility: "direct" }` into a shared constant or ensure all governance configs include the `visibility` field after validation.

### Priority 3: Medium (should fix for code quality)

**R6. Standardize assembly existence checks.** All route handlers that take an assembly ID should verify the assembly exists before proceeding. Consider a shared middleware that validates assembly existence and attaches the assembly info to the context.

**R7. Document all undocumented endpoints.** Update `integration-architecture.md` Section 5.2 to include all implemented endpoints. The documentation should match reality.

**R8. Document the identity model decision.** Whether R1 is implemented or the current model is kept, the decision should be documented in the architecture docs. The current state — where the design docs say one thing and the code does another — is the worst outcome.

### Priority 4: Low (nice to have)

**R9. Add rate limiting middleware.** The VCP architecture (Section 4.3) specifies per-client, per-assembly rate limiting. This is not implemented. For local development this is fine, but it should be added before production deployment.

**R10. Add API versioning.** The integration architecture (Section 8.5) mentions URL-prefix versioning (`/v1/`). This is not implemented. It should be added before any client depends on the API contract.

**R11. Update the seed script for identity separation.** If R1 is implemented, the seed script needs to generate two artifacts: VCP seed data (assemblies, opaque participant IDs, events, delegations, votes, polls) and client seed data (user names, participant ID mappings). The reset script should populate both.
