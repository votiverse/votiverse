...
interface AssetReference {
  ...
  sizeBytes: number;
  hash: string;           // SHA-256 of binary data, hex-encoded (see Appendix C)
...
interface NoteMetadata {
  ...
  contentHash: ContentHash;
  target: NoteTarget;
  endorsementCount: number;       // materialized, updated on each evaluation event
  disputeCount: number;           // materialized, updated on each evaluation event
  status: NoteStatus;
  createdAt: Timestamp;
  withdrawnAt?: Timestamp;
}
...
Endorsement and dispute counts are **materialized** on the note record and updated when `CommunityNoteEvaluated` events are processed (same pattern as tally materialization). Visibility is derived:
...
interface ProposalSubmittedPayload {
  proposalId: ProposalId;       // generated by engine
  issueId: IssueId;
  choiceKey?: string;
  authorId: ParticipantId;
  title: string;
...
CREATE TABLE community_notes (
  ...
  endorsement_count     INTEGER NOT NULL DEFAULT 0,  -- materialized from note_evaluations
  dispute_count         INTEGER NOT NULL DEFAULT 0,  -- materialized from note_evaluations
...
POST   /assemblies/:id/proposals/drafts/:did/submit Submit (→ VCP first, then store content)
...
POST   /assemblies/:id/proposals/:pid/version       Create new version (→ VCP first, then store content)
POST   /assemblies/:id/proposals/:pid/withdraw      Withdraw (→ VCP first)
```

**Candidacies:**
```
POST   /assemblies/:id/candidacies                  Declare (→ VCP first, then store content)
...
POST   /assemblies/:id/candidacies/:cid/version      New version (→ VCP first, then store content)
POST   /assemblies/:id/candidacies/:cid/withdraw     Withdraw (→ VCP first)
```

**Community Notes:**
```
POST   /assemblies/:id/notes                        Create note (→ VCP first, then store content)
GET    /assemblies/:id/notes                         List notes (content + evaluation counts)
...
POST   /assemblies/:id/notes/:nid/withdraw           Withdraw (→ VCP first)
```
...
2. Backend:
   ...
   b. Computes contentHash (see Appendix C)
   c. Resolves user → participantId for assembly
   d. Calls VCP: POST /assemblies/:id/proposals
      { issueId, choiceKey, authorId: participantId, title, contentHash }
   e. VCP validates deliberation window, generates proposalId,
      records ProposalSubmitted event, returns { proposalId, ... }
   f. On VCP success:
      - Stores content in proposal_content keyed by VCP-returned proposalId
...
**VCP-first rule.** The backend never stores submitted content before the VCP confirms acceptance. This applies to all content operations — proposals, candidacies, notes, and new versions. If the VCP rejects, the backend returns the error and local state is unchanged. This eliminates the class of inconsistency where the backend has content the VCP doesn't know about.

**ID generation.** The VCP/engine generates all governance entity IDs (proposalId, candidacyId, noteId). The backend sends metadata without an ID, receives the generated ID in the response, and uses it to key local content storage. This matches the existing pattern for participants, voting events, and delegations.
...
The content hash is a SHA-256 digest that covers both the markdown text and the binary content of all referenced assets. It is computed identically by the backend (at submission time) and can be verified by any party (the VCP, an auditor, a participant).

### Algorithm

```
contentHash = SHA-256(markdown + "\0" + assetHash_1 + "\0" + assetHash_2 + ... )
```

1. Start with the **markdown string** encoded as UTF-8.
2. Append a null byte (`\0`) as separator.
3. For each asset referenced in the content:
   a. Compute `SHA-256(asset binary data)` → hex-encoded string.
4. **Sort** the asset hashes lexicographically (ascending).
5. Join them with null byte separators and append to the string.
6. Compute `SHA-256` of the entire byte sequence → hex-encoded string.

### Edge cases

- **No assets:** `contentHash = SHA-256(markdown + "\0")`. The trailing null byte is always present.
- **Empty markdown:** Valid. `contentHash = SHA-256("\0" + sorted asset hashes)`.
- **Whitespace normalization:** None. The markdown is hashed as-is. Trailing newlines, spaces, and formatting are significant.

### Asset hashes

Each `AssetReference` includes a `hash` field computed at upload time:
...
interface AssetReference {
  ...
  storageKey: string;
  sizeBytes: number;
  hash: string;              // SHA-256 of the binary data, hex-encoded
  uploadedAt: Timestamp;
}
```

The content hash computation uses these stored asset hashes — it does not re-read the binary data. This means the content hash can be computed quickly from metadata alone, while full integrity verification (re-hashing the actual binary) is a separate, heavier operation.
...
function computeContentHash(markdown: string, assetHashes: string[]): ContentHash {
...
## Appendix D: Implementation Principles

### VCP-first rule

The backend never persists submitted content before the VCP confirms acceptance. For every write operation that involves both VCP and backend state:

1. Compute what's needed (contentHash, resolve participantId).
2. Call the VCP. Wait for success.
3. On VCP success: store content locally, keyed by the VCP-returned ID.
4. On VCP rejection: return the error to the client. No local state changes.

This applies to all content operations: proposal submission, proposal versioning, candidacy declaration, candidacy versioning, note creation, note evaluation, and withdrawals. It eliminates the class of inconsistency where the backend has data the VCP doesn't know about.

This principle extends beyond content — it applies to all existing backend-to-VCP interactions (voting, delegations, poll responses). The backend is a proxy and orchestrator; the VCP is the source of truth for governance state.

### ID generation

The VCP/engine generates all governance entity IDs. The backend sends creation requests without IDs and receives generated IDs in the response. This matches the existing pattern for participants, voting events, delegations, and polls.

For **draft proposals**, which exist only in the backend, the backend generates its own draft IDs (UUIDs). These are discarded when the draft is submitted — the VCP assigns the real proposalId.

### No backwards compatibility

The project is pre-production. All changes to types, configs, tests, seed data, and database schemas are clean breaks. There are no migration shims, transition periods, or dual-format validation. When `DelegationConfig.enabled` is replaced by `delegationMode`, it is replaced everywhere in a single pass — presets, tests, engine code, VCP code, and seed data.

### Note evaluation materialization

Endorsement and dispute counts are materialized on the `community_notes` row and updated synchronously when `CommunityNoteEvaluated` events are processed. This follows the same pattern as tally and participation materialization: the event store is the source of truth, the materialized columns are a read optimization that can be recomputed from events if needed.

When a participant changes their evaluation (new event supersedes previous), the VCP:
1. Reads the participant's previous evaluation from `note_evaluations` (if any).
2. Decrements the old count, increments the new count on `community_notes`.
3. Upserts the `note_evaluations` row.
