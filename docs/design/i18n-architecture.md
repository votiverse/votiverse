...
- **SPA architecture**: The app is a single-page application built with Vite v7, React v19, and TypeScript. There is no server-side rendering (SSR) and no static site generation (SSG). All routing is client-side.
- **Auth-gated**: The entire app requires authentication except for the login page. There is a backend with a `user` table that stores each user's locale preference.
- **Current state**: The codebase is English-only with hardcoded strings. It needs to support 20+ languages.
...
| Language detection (pre-auth) | `i18next-browser-languagedetector` | Detects `navigator.language` for the login page before the user is authenticated |
...
| Namespace | Contents |
|---|---|
| `common` | Shared UI: buttons, labels, form validation messages, errors, confirmations, empty states |
| `auth` | Login, signup, password reset, email verification |
| `governance` | Proposals, voting, delegates, delegation, surveys, community notes |
| `settings` | Account settings, preferences, profile, notifications |
...
// Injected into the SPA at initial load (e.g., from auth context or a global config).
...
Wrap the app in a Suspense boundary so the initial locale file can load before rendering.
...
// For pre-auth (login page), call with no argument.
// For authenticated pages, pass the user's locale from the backend.
async function bootstrap(userLocale?: string) {
...
Once the user is authenticated and their profile (including locale) is available:

```tsx
// Example: inside your auth initialization flow

const userProfile = await fetchUserProfile(); // { locale: 'fr', ... }
await initI18n(userProfile.locale);
```

The exact wiring depends on the existing auth flow. The key constraint is: **i18next must be initialized with the user's locale before React renders any translated content.**
...
|---|---|---|
| Authenticated user | Backend `user` table | `locale` column, returned in user profile on login/reload |
| Pre-auth (login page) | Browser | `navigator.language`, cached in `localStorage` |
| Language switch | Backend | PATCH user profile → reload page |

The backend is the single source of truth for authenticated users. Do not store the locale in JWT (immutable until reissued) or cookies (parallel state channel). The backend sends the locale as part of the user profile payload, and the SPA initializes i18next with that value.
...
Location: `scripts/i18n/translation-prompt.md`
...
The single source of truth for which locales exist is a plain JSON file at the project root. This file is consumed by the app (via import), the translation script, CI, and any other tooling.
...
- Tailwind's `rtl:` variant handles most layout mirroring if Tailwind is configured with `dir` support.
...
Set this during app bootstrap, alongside i18next initialization.

## Migration Steps

To migrate the existing English-only codebase:

1. **Install dependencies** and create the `src/i18n.ts` config as specified above.
2. **Create `public/locales/en/common.json`** by extracting all user-facing strings from the codebase. Start with `common`, add other namespaces as patterns emerge.
3. **Replace hardcoded strings** with `t()` calls, component by component. Prioritize shared components first (buttons, modals, form labels), then move to domain views.
4. **Wire initialization** into the existing bootstrap/auth flow.
5. **Add the language picker** to the settings page.
6. **Run the AI translation pipeline** to generate the initial set of locale files from `en`.
7. **Test with a high-contrast language** (e.g., Japanese or Arabic) to catch layout issues and hardcoded strings that were missed.

## Summary

- **App type**: SPA (Vite v7 + React v19 + TypeScript), no SSR/SSG, auth-gated
...
- **Namespaces**: By domain concern, not by route
- **Keys**: Flat with dot notation
- **Locale source of truth**: Backend user table (pre-auth: browser detection)
- **Language switch**: PATCH backend → `window.location.reload()`
- **Fallback**: Regional variant → base language → English
- **Supported languages**: `locales.json` at project root — single source of truth for app, scripts, and CI
- **Translation files**: Flat JSON, optimized for AI pipelines
- **Staleness detection**: Content-addressable hashing with per-locale `_manifest.json`
- **Translation script**: `scripts/i18n/translate.ts` with `--locale`, `--namespace`, and `--dry-run` flags
- **LLM prompt**: Stored as `scripts/i18n/translation-prompt.md`, separate from script code
