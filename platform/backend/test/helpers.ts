...
export async function createTestBackend(): Promise<TestBackend> {
  ...
  const app = createApp({ database: db, userService, sessionService, membershipService, assemblyCacheService, topicCacheService, notificationService, config: TEST_CONFIG });
  ...
  return { app, db, userService, sessionService, assemblyCacheService, topicCacheService, cleanup, request, registerAndLogin };
}
