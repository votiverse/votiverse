...
export function BadgeDot({ count, className = "" }: { count: number; className?: string }) {
  ...
  return (
    <span className={`inline-flex items-center justify-center min-w-[18px] h-[18px] px-2 rounded-full bg-error-subtle text-error-text text-[10px] font-bold leading-none ${className}`}>
      {count > 99 ? "99+" : count}
    </span>
  );
}
